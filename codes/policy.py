...
class MlpPolicy(nn.Module):
    ...
    def init_parameters(self): # questionable
    ...
    def forward(self, obs, sample = True, fixed_action = None):
        ...
        obs = obs.view(-1) # wired design, seems that the batchsize will always be 1
        ...
        if fixed_action is not None: # bad design
        ...
        return action.item(), policy.log_prob(action) # a, log P(a|s)
...
class DiagonalGaussianMlpPolicy(nn.Module):
    def __init__(self,
        ...
        # store parameters
        self.activation = activation
        self.output_activation = output_activation # useless
