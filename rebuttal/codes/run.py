...
def run(opts):
    ...
    opts.no_saving = True  # TODO: danger, but we do not want to save checkpoints
    ...
    # Do validation only
    if opts.eval_only: # validation == evaluation
        # Set the random seed
        torch.manual_seed(opts.seed)
        np.random.seed(opts.seed)
        
        # Load data from load_path
        assert opts.load_path is not None, "You have to specify the model to validate."
        ...
        for run_id in opts.seeds:
            # Set the random seed, actually gym and python itself also requires seed setup
            ...
            model_actor = get_inner_model(agent.master.logits_net) # central server agent
